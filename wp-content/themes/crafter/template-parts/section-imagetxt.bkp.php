...
<div id="video-section" class="video-section" <?php if( false == $crafter_enable_section ): echo 'style="display: none;"'; endif ?>>
    ...
    ?>
    <div class="bullet-points-image wow fadeInRight" data-wow-delay="300ms" style="background-image: url(<?php echo esc_url( $bullet_points_image ) ?>);">
        <h3 class="bullet-points-title"><?php  echo esc_html( get_theme_mod( 'crafter_image_ttl_title' ) ); ?></h3>
        ...
        $main_text = wp_kses( get_theme_mod( 'crafter_image_main_desc' ), $wp_kses_args );
        ?>
        <p><?php echo $main_text; ?></p>
        <?php $crafter_bullet_points_link_title = get_theme_mod( 'crafter_image_link_title', esc_html__( 'Learn More', 'crafter' ) ); ?>
        <?php if ( !empty( $crafter_bullet_points_link_title ) || is_customize_preview() ) { ?>
        <a href="<?php echo esc_url( get_theme_mod( 'crafter_image_link_url', '#' ) ); ?>" class="btn-ql"><?php echo esc_html( $crafter_bullet_points_link_title ); ?></a>
        <?php } ?>
    </div>
